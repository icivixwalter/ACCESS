Attribute VB_Name = "COLLEGA_Mdl01_LLPP_DF12_TIPO_ATTO"
...
'//########################################################################################################################################################//
Public Function COLLEGA_Mcr_LLPP_ATTI_Tb01_Gestione()
...
'//blocco macro
Dim MsgBloccMcr_s As String
Dim BloccMcr_b  As Boolean
...
PathOrigine_s = "c:\GESTIONI\GESTIONE_LLPP\25_GESTIONE_LLPP\LLPP_ARCHIVI_MDB\"
        ...
        '//Parametri da inserire : acLink = Tabella collegata _
                ...
                DatabaseOrigine_s = "LLPP_ATTI_DEFINIZIONI.mdb"
                PathDatabaseOrigine_s = PathOrigine_s & DatabaseOrigine_s

                
               '//MESSAGGIO DI BLOCCO DELLA MACRO
               '//---------------------------------------------------------------------------------------------------//
                '//Messaggio di blocco applicato nel database originale, con il quale _
                escludi dalla procedura corrente la macro attivata in questo database originale _
                per evitare di cancellare le tabelle originali.
                
                    '//imposto il blocco della macro
                    BloccMcr_b = False
                    MsgBloccMcr_s = "BLOCCO DELLA MACRO CORRENTE, per evitare la cancellazione" & Chr$(13) _
                                    & "delle tabelle correnti. Per rimuovere il blocco impostare la " & Chr$(13) _
                                    & "da True a FALSE"
                    If BloccMcr_b = True Then
                        MsgBox MsgBloccMcr_s, vbCritical, "BLOCCO DELLA MACRO CORRENTE"
                        'Stop
                        GoTo COLLEGA_Mcr_LLPP_ATTI_Tb01_Gestione_Exit
                    End If
                ...
                DoCmd.TransferDatabase acLink, "Microsoft Access", _
                        ...
                        '//---------------------------------------------------------------------------------------------------//
                            ...
                            '//IMPOSTAZIONE DELLE VARIABILI _
                            reimposto sia la tabelladi origine che di destinazione. Quest'ultima può essere anche ridenominata.
                            TabellaOrigine_s = "LLPP_DF12_}----------------------------------------------------@"
                            TabellaDestinazione_s = "LLPP_DF12_}----------------------------------------------------@"
