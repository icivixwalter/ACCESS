...
Dim sTblQryName As String
    ...
    Call CreateSchemaFile(bIncFldNames, sSectionName, sPath, sTblQryName)
...
Private Sub COLLEGA_FILE_TESTO_DELIMITATO()
    ...
    Set Dbs = CurrentDb
    Set Tbf = Dbs.CreateTableDef("TestoProdotti")
    Tbf.Connect = "TEXT;DATABASE=c:\casa\"
    Tbf.SourceTableName = "Prodotti.txt"
...
Dim Dbs As DAO.Database
Set Dbs = CurrentDb
Dbs.TableDefs.Delete ("TestoProdotti")
End Sub
...
Dim tbl As ADOX.Table
    ...
    tbl.Properties("Jet OLEDB:Link Datasource") = "C:\Casa\CONSULENTE_FITOSANITARI.mdb"
...
Dim Tbf As DAO.TableDef
    ...
    '//Apre un database diverso da quello corrente, _
       ...
       Set Dbs = OpenDatabase("C:\Casa\Db.mdb")
...
'//Questa routine apre un recordse su glio di lavoro SocietaAlfa _
   ...
   che si trova nella directory c:\CASA\. Successivamente conta il _
